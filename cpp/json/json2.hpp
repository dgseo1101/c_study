...
namespace namespace_json_2 {
	...
	std::ostream& operator<<(std::ostream& os, const JValue* v) {
		return v->Repr(os); // 출력할때, 주소값이 아닌 value 값을 뱉어내주기 위해
	}
	...
	class JObject : public JValue, public std::map<std::string, JValue*> {
		static inline bool checkName(const std::string& name){
			return std::all_of(name.cbegin(), name.cend(), isalpha);
		}
	public:
		...
		void Set(const std::string& key, JValue* v){
			auto it = find(key);
			if (it == end()){
				insert(std::make_pair(key, v)); // make_pair = 두 객체를 하나의 객체로 취급할 수 있게끔 묶어두는 함수
			}
			else {
				delete it->second;
				it->second = v;
...
#define JSONLIB_THROW_ERROR(what) json_parse_error(__FUNCTION__, what, is.tellg())
	...
	JValue* JValue::Parse(std::istream& is)
	{
		...
		default: //else: number, symbolic literal, error
		{
			if (std::isdigit(c)) { //number
				...
				v = JLiteral::Parse(is); // "true", "false", "null";
			}
			else {
	...
	JArray* JArray::Parse(std::istream& is)
	{
		...
		while (is.good()){
			...
			SkipSpaces(is);
			try {
				v = JValue::Parse(is); // array니 key값 없이 value만 파싱해서 push_back
				arr->push_back(v);
			}
	...
	JObject* JObject::Parse(std::istream& is)
	{
		...
		while (is.good()){ // 입출력 가능 상태 판별
			JValue* v = nullptr;
			SkipSpaces(is); // 공백 제거
			try {
				std::string key;
				auto bg = is.peek(); // 다음 문자를 읽기만 함
				if (bg == '\'' || bg == '"') { 
					key = JString::ParseString(is); // String parsing -> 유니코드, \t 와 같은 예외처리
					if (ReadSkipSpaces(is) != ':')
						throw JSONLIB_THROW_ERROR("':' 없음"); // value가 없을때 예외처리
				}
				else {
					std::getline(is, key, ':');
					if (is.eof())
				...
				SkipSpaces(is);
				v = JValue::Parse(is); // value값 parsing
				obj->Set(key, v); // key, value 묶어서 등록
			}
			catch (std::exception& e){
				if (v)
					delete v;
    ...
    std::string find(JValue* org, const std::string& select, char delim = ',') {
		std::vector<std::string> path = tokenize_selector(select, delim);
		std::string buf;
        auto temp = org;
		...
		for (const auto& p : path) {
			if (org->type == VALUE_TYPE::OBJECT){
				temp = static_cast<JObject*>(org)->at(p);
                buf += temp->to_string();
                buf += delim;
            }
			else if (org->type == VALUE_TYPE::ARRAY) {
				size_t idx = std::stoi(p);
				temp = static_cast<JArray*>(org)->at(idx);
                buf += temp->to_string();
                buf += delim;
			}
			else {
